# Brand logo gaps

Generated: 2026-08-24T00:00:00.000Z

- SAFE brands: 2354
- SAFE with publishable identity-matched logo: 1988
- SAFE without logo: 366
- Registry mappings excluded because identity is not SAFE: 36

## Priority distribution

- HIGH: 363
- NORMAL: 3

## First 100 gaps

- Elco (`elco-motors`) — elco-spa.com
- HERZ (`herz`) — herz.eu
- MP Pumps (`mp-pumps`) — mppumps.com
- Thomson (`thomson`) — thomsonlinear.com
- Lutz Pumpen (`lutz-pumpen-gmbh`) — lutzpumps.com
- Duplomatic (`duplomatic`) — duplomaticms.com
- Franklin Electric (`franklin-electric`) — franklinwater.com
- Numatics (`asco-numatics`) — discreteautomation.emerson.com
- SIMOTOP (`simotop`) — simotopgroup.com
- Gruner (`gruner-ag`) — gruner.de
- DUNGS (`dungs`) — dungs.com
- Pedrollo (`pedrollo`) — pedrollo.com
- APV (`apv`) — spxflow.com
- Liverani (`liverani`) — liverani.com
- Lowara (`lowara`) — xylem.com
- Hamworthy (`hamworthy`) — hamworthy.com
- Waukesha Cherry-Burrell (`waukesha`) — spxflow.com
- Champion Pneumatic (`champion`) — championpneumatic.com
- Gefran (`gefran`) — gefran.com
- Mono Pumps (`mono-pumps-limited`) — mono-pumps.com
- Ascon Tecnologic (`ascon-tecnologic`) — ascontecnologic.com
- co-ax (`co-ax-valves-inc`) — co-ax.com
- ipf electronic (`ipf-electronic`) — ipf-electronic.de
- Pneumax (`pneumax`) — pneumaxspa.com
- KROMBACH (`krombach`) — cranecpe.com
- KABELSCHLEPP (`kabelschlepp`) — tsubaki-kabelschlepp.com
- Kadant Johnson (`kadant-johnson`) — fluidhandling.kadant.com
- WAMGROUP (`wamgroup`) — wamgroup.com
- Johnson Pump (`johnson-pump`) — spxflow.com
- TECOFI (`tecofi`) — tecofi.fr
- NASH (`nash`) — nashpumps.com
- SONDERMANN (`sondermann`) — sondermann-pumpen.de
- SIATA (`siata`) — pentair.eu
- Foxboro Eckardt (`foxboro-eckardt`) — se.com
- Bran+Luebbe (`bran-luebbe-gmbh`) — spxflow.com
- HEINZMANN (`heinzmann`) — heinzmann.com
- SOFIMA Hydraulics (`sofima`) — ufihyd.com
- Dynisco (`dynisco`) — dynisco.com
- Hankison (`hankison`) — hankisonair.com
- Jabsco (`jabsco-pump`) — xylem.com
- LTN Servotechnik (`ltn-servotechnik`) — heidenhain.de
- TSCHAN (`tschan-gmbh`) — ringfeder.com
- Welch (`welch-vacuum`) — welchvacuum.com
- GEMELS (`gemels`) — gemels.com
- Nicotra Gebhardt (`nicotra`) — nicotra-gebhardt.com
- Verderflex (`verderflex`) — verderliquids.com
- Hurco (`hurco`) — hurco.com
- NAVCO (`navco`) — navco.us
- TOREX (`torex-spa`) — wamgroup.co.uk
- DESMI (`desmi`) — desmi.com
- LÜBBERING (`lubbering-luebbering`) — luebbering.de
- OLAER (`olaer-group`) — parker.com
- Stone Hydraulics (`stone`) — hydraulictechnologies.com
- Control Techniques (`control-techniques`) — moen.nidec.com
- Microprecision Electronics (`microprecision-electronics`) — microprecision.ch
- Bekum (`bekum`) — bekum.com
- INDEX TRAUB (`traub`) — index-group.com
- Lightnin (`lightnin`) — spxflow.com
- Alemite (`alemite`) — skf.com
- ARON (`aron`) — dana.com
- Bradman Lake (`bradman_lake`) — bradmanlake.com
- Concens (`concens`) — concens.com
- FRIEDRICH Schwingtechnik (`friedrich-schwingtechnik-gmbh`) — friedrich-schwingtechnik.de
- GF Machining Solutions (`gf-agie-charmilles`) — machining.com
- GSM (`gsm`) — stmspa.com
- Hanovia (`hanovia`) — hanovia.com
- Haskel (`haskel`) — haskel.com
- Lamson (`lamson`) — hoffmanandlamson.com
- EMP Slavkov (`slavkov`) — emp-slavkov.cz
- Yılmaz Redüktör (`yilmaz`) — yr.com.tr
- Balston (`balston`) — parker.com
- Berger Lahr (`berger-lahr`) — se.com
- BEVI (`bevi`) — bevi.com
- CompAir (`compair`) — compair.com
- HSME (`hsme`) — hsmecorp.com
- PROCOM Valves (`procom-valves`) — procomcontrolfluid.com
- RIEGLER (`riegler`) — riegler.de
- UNI-Geräte (`uni-geraete`) — uni-geraete.com
- WINKEL (`winkel`) — winkel.de
- HAHN Gasfedern (`hahn-gasfedern`) — hahn-gasfedern.de
- Balkansko Echo (`balkansko-echo`) — balkanskoecho.com
- STERLING SIHI (`sterling-sihi`) — sterling-pumps.com
- Tamrotor (`tamrotor`) — ingersollrand.com
- Wollin (`wollin`) — wollin.de
- Blagdon Pump (`blagdon-pump`) — blagdonpumps.co.uk
- PRUFTECHNIK (`pruftechnik`) — pruftechnik.com
- Robuschi (`robuschi`) — robuschi.com
- SEEPEX (`seepex`) — seepex.com
- Electro Adda (`electro-adda`) — electroadda.com
- KELLER Pressure (`keller`) — keller-pressure.com
- Motovario (`motovario`) — motovario.com
- Tollok (`tollok`) — rexnord.com
- WEINREICH (`weinreich`) — weinreich.de
- Aeromeccanica Stranich (`aeromeccanica-stranich`) — stranich.it
- Dean Pump (`dean-pump`) — cecoenviro.com
- Hannemann & Pruss (`waldemar-pruss`) — pruss.de
- Exlar (`exlar`) — cw-actuation.com
- Hydrovane (`hydrovane`) — compair.com
- IDAM (`idam`) — schaeffler.com
- Mecafrance (`mecafrance`) — mecafrance-bonn.de

The full machine-readable queue is `data/brand-logo-gaps.json`. A missing logo never lowers identity verification, and a logo is never assigned through fuzzy matching alone.
