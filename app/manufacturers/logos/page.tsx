  ...
  return (
    <>
      ...
      <section className="section shell">
        <div className="logo-source-list">{brandLogoRegistry.map((logo) => <article className="logo-source-card" key={logo.slug}><div className={`logo-source-preview${logo.presentationBackground === "dark" ? " logo-on-dark" : ""}`}><Image alt={`${logo.name} logo`} height={120} src={logo.src} unoptimized width={260} /></div><div><h2>{logo.name}</h2><p>{logo.license}</p><p>{logo.attribution}</p>{logo.licenseUrl && <a href={logo.licenseUrl} rel="noreferrer" target="_blank">Licence terms ↗</a>}<a href={logo.sourcePage} rel="noreferrer" target="_blank">Source page ↗</a></div></article>)}</div>
